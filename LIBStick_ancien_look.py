...
# gestion des zooms
minimum_spectre_L_trait = maximum_spectre_L_trait = maximum_spectre_ancien_L_trait = maximum_spectre_corrige_ancien_L_trait = 0
maximum_spectre_ancien_L_ext = 0
maximum_spectre_ancien_L_comp = 0
maximum_spectre_ancien_L_ACP = 0
anciennes_zoom_inf_L_trait = anciennes_zoom_sup_L_trait = 0
anciennes_zoom_inf_L_ext = anciennes_zoom_sup_L_ext = 0
anciennes_zoom_inf_L_comp = anciennes_zoom_sup_L_comp = 0
anciennes_zoom_inf_L_ACP = anciennes_zoom_sup_L_ACP = 0
...
# def applique_change_taille_fenetre_event(event) :
#     applique_change_taille_fenetre()
...
limites_spectre_L_trait = np.array([198.0, 1013.0])
...
def lit_limites_abscisses_L_trait(spectre):
    ...
    limites_spectre = np.zeros((2))
    limites_spectre[0] = tableau_abscisses[0]             # lit les valeurs min et max du spectre
...
def lit_affiche_spectre_numero_L_trait():
    ...
    spectre_entier_L_trait = LIBStick_outils.lit_spectre(
        nom_fichier_seul_L_trait, type_fichier_L_trait.get())
    affiche_spectre_L_trait()
...
def affiche_spectre_corrige_L_trait():
    ...
    if flag_zoom_auto_y.get() is False and flag_dezoom_L_trait is False and flag_bouton_zoom_L_trait is False:
        ...
        maximum = (maximum_spectre_corrige_ancien_L_trait -
                   minimum_spectre_corrige_ancien_L_trait) * (hauteur_canevas_spectres-coord_zoom_L_trait[1])/hauteur_canevas_spectres
...
def zoom_drag_and_drop_L_trait(event):
    ...
    if coord_zoom_L_trait[2] > coord_zoom_L_trait[0]:
        ...
        debut = coord_zoom_L_trait[0]*delta_limites_L_trait / \
            largeur_canevas_spectres+limites_affichage_spectre_L_trait[0]
...
# initialisations
###################################################################################################
limites_spectre_L_ext = np.array([198.0, 1013.0])
...
def affiche_spectre_L_ext():
    ...
    if flag_zoom_auto_y.get() is False and flag_dezoom_L_ext is False and flag_bouton_zoom_L_ext is False:
        ...
        maximum = (maximum_spectre_ancien_L_ext-minimum_spectre_ancien_L_ext) * \
            (hauteur_canevas_spectres-coord_zoom_L_ext[1])/hauteur_canevas_spectres
        maximum_spectre_ancien_L_ext = maximum
    ...
    if flag_zoom_auto_y.get() is False and flag_dezoom_L_ext is False and flag_bouton_zoom_L_ext is True:
        maximum = maximum_spectre_ancien_L_ext
        limites_affichage_spectre_L_ext[0] = variable_zoom_inf_L_ext.get()
        limites_affichage_spectre_L_ext[1] = variable_zoom_sup_L_ext.get()
        delta_limites_L_ext = limites_affichage_spectre_L_ext[1]-limites_affichage_spectre_L_ext[0]
        canevas0_L_ext.delete("all")
    ...
    if flag_zoom_auto_y.get() is True or flag_dezoom_L_ext is True:
        ...
        for ligne in spectre_entier_L_ext:
            if (ligne[0] >= limites_affichage_spectre_L_ext[0] and ligne[0] <= limites_affichage_spectre_L_ext[1]):
                spectre = np.row_stack((spectre, ligne))
        minimum = spectre[:, 1].min()
        maximum_spectre_ancien_L_ext = maximum = spectre[:, 1].max()
...
###################################################################################################
# limites min et max de l'affichage du spectre
limites_spectre_L_comp = np.array([198.0, 1013.0])
# limites de l'affichage du spectre à l'écran
...
def affiche_spectre_L_comp():
    ...
    if flag_zoom_auto_y.get() is False and flag_dezoom_L_comp is False and flag_bouton_zoom_L_comp is False:
        ...
        maximum = (maximum_spectre_ancien_L_comp-minimum_spectre_ancien_L_comp) * \
            (hauteur_canevas_spectres-coord_zoom_L_comp[1])/hauteur_canevas_spectres
        maximum_spectre_ancien_L_comp = maximum
    ...
    if flag_zoom_auto_y.get() is False and flag_dezoom_L_comp is False and flag_bouton_zoom_L_comp is True:
        maximum = maximum_spectre_ancien_L_comp
        limites_affichage_spectre_L_comp[0] = variable_zoom_inf_L_comp.get()
        limites_affichage_spectre_L_comp[1] = variable_zoom_sup_L_comp.get()
        delta_limites_L_comp = limites_affichage_spectre_L_comp[1] - \
            limites_affichage_spectre_L_comp[0]
    ...
    if flag_zoom_auto_y.get() is True or flag_dezoom_L_comp is True:
        ...
        for ligne in spectre_entier_L_comp:
            if (ligne[0] >= limites_affichage_spectre_L_comp[0] and ligne[0] <= limites_affichage_spectre_L_comp[1]):
                spectre = np.row_stack((spectre, ligne))
        minimum = spectre[:, 1].min()
        maximum_spectre_ancien_L_comp = maximum = spectre[:, 1].max()
...
def coord1_to_vars_5_6_L_comp(x, y):
    ...
    variable_5_L_comp.set(format(
        limites_spectre_L_comp[0] + (x * (limites_spectre_L_comp[1]-limites_spectre_L_comp[0]) / largeur_canevas_spectres), "4.1f"))
...
def vars_5_6_to_coord1_L_comp():
    ...
    )-limites_spectre_L_comp[0])*largeur_canevas_spectres) / (limites_spectre_L_comp[1]-limites_spectre_L_comp[0]))
...
# limites min et max de l'affichage du spectre
limites_spectre_L_ACP = np.array([198.0, 1013.0])
# limites de l'affichage du spectre à l'écran
...
def affiche_spectre_L_ACP():
    ...
    if flag_zoom_auto_y.get() is False and flag_dezoom_L_ACP is False and flag_bouton_zoom_L_ACP is False:
        ...
        maximum_spectre_ancien_L_ACP = maximum
    ...
    if flag_zoom_auto_y.get() is False and flag_dezoom_L_ACP is False and flag_bouton_zoom_L_ACP is True:
        maximum = maximum_spectre_ancien_L_ACP
        limites_affichage_spectre_L_ACP[0] = variable_zoom_inf_L_ACP.get()
        limites_affichage_spectre_L_ACP[1] = variable_zoom_sup_L_ACP.get()
        delta_limites_L_ACP = limites_affichage_spectre_L_ACP[1]-limites_affichage_spectre_L_ACP[0]
        canevas0_L_ACP.delete("all")
    ...
    if flag_zoom_auto_y.get() is True or flag_dezoom_L_ACP is True:
        ...
        for ligne in spectre_entier_L_ACP:
            if (ligne[0] >= limites_affichage_spectre_L_ACP[0] and ligne[0] <= limites_affichage_spectre_L_ACP[1]):
                spectre = np.row_stack((spectre, ligne))
        minimum = spectre[:, 1].min()
        maximum_spectre_ancien_L_ACP = maximum = spectre[:, 1].max()
...
def ouvre_fenetre_change_tree_label_L_ACP(event):
    ...
    if tableau_label_ouvert_flag_L_ACP is False:
        ...
        frame_label_L_ACP = tk.Frame(fenetre_label_L_ACP, bg=COULEUR_INTERFACE)
        frame_label_L_ACP.pack()
        label_L_ACP.set(value=item[3])
        entree_label_L_ACP = ttk.Spinbox(
    ...
    for ligne_tableau in DataFrame_element_L_ele.itertuples():
        if float(ligne_tableau[2]) > limites_affichage_spectre[0] and float(ligne_tableau[2]) < limites_affichage_spectre[1]:
            ...
            if intensite_relative >= 10 and flag_sup10_L_ele.get() == 1:
                ...
                if ID_onglet == 0:
                    canevas0_L_trait.create_line(
                        x_ligne, 0, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 1))
                    canevas1_L_trait.create_line(
                        x_ligne, 0, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 1))
                if ID_onglet == 1:
                    canevas0_L_ext.create_line(
                        x_ligne, 0, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 1))
                if ID_onglet == 2:
                    canevas0_L_comp.create_line(
                        x_ligne, 0, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 1))
                if ID_onglet == 3:
                    canevas0_L_ACP.create_line(
                        x_ligne, 0, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 1))
                    canevas1_L_ACP.create_line(
                        x_ligne, 0, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 1))
            if intensite_relative < 10 and intensite_relative >= 1 and flag_sup1_L_ele.get() == 1:
                ...
                if ID_onglet == 0:
                    canevas0_L_trait.create_line(
                        x_ligne, 100, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 2))
                    canevas1_L_trait.create_line(
                        x_ligne, 100, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 2))
                if ID_onglet == 1:
                    canevas0_L_ext.create_line(
            ...
            if intensite_relative < 1 and flag_inf1_L_ele.get() == 1:
                ...
                if ID_onglet == 0:
                    canevas0_L_trait.create_line(
                        x_ligne, 165, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 3))
                    canevas1_L_trait.create_line(
                        x_ligne, 165, x_ligne, hauteur_canevas_spectres, fill=couleur_lignes, dash=(4, 3))
                if ID_onglet == 1:
                    canevas0_L_ext.create_line(
...
def ouvre_fenetre_classification_L_ele():
    ...
    if tableau_periodique_ouvert_L_ele is False:
        ...
        frame1_L_ele = tk.Frame(fenetre_classification_L_ele, bg=COULEUR_INTERFACE)
        frame1_L_ele.pack()

        bouton_ferme_L_ele = tk.Button(frame1_L_ele, width=TAILLE_CASE[0]*2, height=TAILLE_CASE[1],
        ...
        coche_El_I_L_ele = tk.Radiobutton(frame1_L_ele, text=_("Neutres"),
                                               variable=flag_neutres_ions_L_ele, value=1, bg=COULEUR_INTERFACE,
                                               font=font.Font(size=TAILLE_FONT_CLASSIFICATION),
                                               command=affiches_lignes_neutres_ions_L_ele)
        coche_El_I_L_ele.grid(row=1, column=5, columnspan=3, sticky=tk.W)
        coche_El_II_L_ele = tk.Radiobutton(frame1_L_ele, text=_("Ions +"),
                                                variable=flag_neutres_ions_L_ele, value=2, bg=COULEUR_INTERFACE,
                                                font=font.Font(size=TAILLE_FONT_CLASSIFICATION),
                                                command=affiches_lignes_neutres_ions_L_ele)
        coche_El_II_L_ele.grid(row=2, column=5, columnspan=3, sticky=tk.W)

        coche_sup10_L_ele = tk.Checkbutton(frame1_L_ele, text=_("I relative >= 10%"),
                                                variable=flag_sup10_L_ele, bg=COULEUR_INTERFACE,
                                                font=font.Font(size=TAILLE_FONT_CLASSIFICATION),
                                                command=affiches_lignes_element_L_ele)
        coche_sup10_L_ele.grid(row=1, column=9, columnspan=4, sticky=tk.W)
        coche_sup1_L_ele = tk.Checkbutton(frame1_L_ele, text=_("1% <= I relative < 10%"),
                                               variable=flag_sup1_L_ele, bg=COULEUR_INTERFACE,
                                               font=font.Font(size=TAILLE_FONT_CLASSIFICATION),
                                               command=affiches_lignes_element_L_ele)
        coche_sup1_L_ele.grid(row=2, column=9, columnspan=4, sticky=tk.W)
        coche_inf1_L_ele = tk.Checkbutton(frame1_L_ele, text=_("I relative < 1%"),
                                               variable=flag_inf1_L_ele, bg=COULEUR_INTERFACE,
                                               font=font.Font(size=TAILLE_FONT_CLASSIFICATION),
                                               command=affiches_lignes_element_L_ele)
...
fenetre_principale =ThemedTk(theme='plastik')
...
# fenetre_principale.maxsize(width=largeur_fenetre_principale,
                           # height=hauteur_fenetre_principale)

# vscrollbar = AutoScrollbar(fenetre_principale, orient=tk.VERTICAL, bg=COULEUR_INTERFACE)
# vscrollbar.grid(row=0, column=1, sticky=tk.N+tk.S)
# hscrollbar = AutoScrollbar(fenetre_principale, orient=tk.HORIZONTAL, bg=COULEUR_INTERFACE)
...
frame_scroll = tk.Frame(canevas_scroll, bg=COULEUR_INTERFACE)
frame_scroll.rowconfigure(1, weight=1)
frame_scroll.columnconfigure(1, weight=1)

onglets = ttk.Notebook(frame_scroll)
onglets.pack()
onglet1 = tk.Frame(onglets, bg=COULEUR_INTERFACE)
onglet2 = tk.Frame(onglets, bg=COULEUR_INTERFACE)
onglet3 = tk.Frame(onglets, bg=COULEUR_INTERFACE)
onglet4 = tk.Frame(onglets, bg=COULEUR_INTERFACE)
...
barre_menus = tk.Menu(fenetre_principale, bg=COULEUR_INTERFACE)
menu_fichier = tk.Menu(barre_menus, bg=COULEUR_INTERFACE)
menu_traitement = tk.Menu(barre_menus, bg=COULEUR_INTERFACE)
menu_extraction = tk.Menu(barre_menus, bg=COULEUR_INTERFACE)
menu_comparaison = tk.Menu(barre_menus, bg=COULEUR_INTERFACE)
menu_outils = tk.Menu(barre_menus, bg=COULEUR_INTERFACE)
sous_menu_langue = tk.Menu(menu_outils, bg=COULEUR_INTERFACE)
...
###################################################################################################
frame1_L_trait = tk.Frame(onglet1, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
frame2_L_trait = tk.Frame(onglet1, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
...
lambda_texte_L_trait = tk.Label(
    frame1_L_trait, text="Lambda = " + str(format(lambda_texte_spectre_0_L_trait, "4.1f") + " nm"), bg=COULEUR_INTERFACE)
lambda_texte_L_trait.grid(row=2, column=5)

text1_L_trait = tk.Label(frame1_L_trait, text=_("Borne inf :"), bg=COULEUR_INTERFACE)
text2_L_trait = tk.Label(frame1_L_trait, text=_("Borne sup :"), bg=COULEUR_INTERFACE)
...
text3_L_trait = tk.Label(frame1_L_trait, text=_("Filtre :"), bg=COULEUR_INTERFACE)
text4_L_trait = tk.Label(frame1_L_trait, text=_("Taille :"), bg=COULEUR_INTERFACE)
text5_L_trait = tk.Label(frame1_L_trait, text=_("Ordre :"), bg=COULEUR_INTERFACE)
text5_2_L_trait = tk.Label(frame1_L_trait, text=_("Dérivée :"), bg=COULEUR_INTERFACE)
...
entree4_L_trait = ttk.Spinbox(frame1_L_trait, from_=3, to=199,
...
text6_L_trait = tk.Label(frame1_L_trait, text=_("Fond :"), bg=COULEUR_INTERFACE)
text7_L_trait = tk.Label(frame1_L_trait, text=_("Itérations :"), bg=COULEUR_INTERFACE)
text8_L_trait = tk.Label(frame1_L_trait, text="LLS :", bg=COULEUR_INTERFACE)
...
entree7_L_trait = ttk.Spinbox(frame1_L_trait, from_=3, to=100,
                                  textvariable=param1_fond_L_trait, width=5)
entree8_L_trait = ttk.Spinbox(frame1_L_trait, from_=1, to=100,
                                  textvariable=param2_fond_L_trait, width=5)
entree8bis_L_trait = tk.Checkbutton(
...
frame1_1_L_trait = tk.Frame(frame1_L_trait, bg=COULEUR_INTERFACE)
frame1_1_L_trait.grid(row=1, column=7, rowspan=4, sticky=tk.N+tk.S)

text_zoom_L_trait = tk.Label(frame1_1_L_trait, text="Zoom : ", width=8, bg=COULEUR_INTERFACE)
...
bouton_rep_L_trait = tk.Button(frame1_1_L_trait, text=_("Fichier"),
                                    command=choix_fichier_L_trait, width=8, bg=COULEUR_INTERFACE)
bouton_visualisation_L_trait = tk.Button(frame1_1_L_trait, text=_("Visualisation"),
...
frame2_1_L_trait = tk.Frame(frame2_L_trait, bg=COULEUR_INTERFACE)
...
coche_tous_fichiers_L_trait = tk.Checkbutton(frame2_1_L_trait, text=_("Appliquer sur\ntous les fichiers\ndu répertoire"),
...
coche_sauve_fond_L_trait = tk.Checkbutton(frame2_1_L_trait, text=_("Sauvegarde du\nfond continu"),
                                               variable=flag_sauve_fond_L_trait, bg=COULEUR_INTERFACE)
coche_sauve_fond_L_trait.grid(row=2, column=1)

bouton_execute_L_trait = tk.Button(frame2_1_L_trait, text=_("Executer"), state="disable",
                                        command=execute_L_trait, width=8, bg=COULEUR_INTERFACE)
bouton_execute_L_trait.grid(row=3, column=1)

text_spectre_L_trait = tk.Label(frame2_1_L_trait, text=_("Spectre : "), bg=COULEUR_INTERFACE)
...
###################################################################################################
frame1_L_ext = tk.Frame(onglet2, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
frame2_L_ext = tk.Frame(onglet2, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
frame3_L_ext = tk.Frame(onglet2, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
...
lambda_texte_L_ext = tk.Label(
    frame1_L_ext, text="Lambda = " + str(format(lambda_texte_spectre_L_ext, "4.1f") + " nm"), bg=COULEUR_INTERFACE)
lambda_texte_L_ext.grid(row=2, column=5)

text1_L_ext = tk.Label(frame1_L_ext, text=_("Première borne inf (nm)"), bg=COULEUR_INTERFACE)
text2_L_ext = tk.Label(frame1_L_ext, text=_("Première borne sup (nm)"), bg=COULEUR_INTERFACE)
text3_L_ext = tk.Label(frame1_L_ext, text=_("Seconde borne inf (nm)"), bg=COULEUR_INTERFACE)
text4_L_ext = tk.Label(frame1_L_ext, text=_("Seconde borne sup (nm)"), bg=COULEUR_INTERFACE)
...
# bouton_rep_L_ext=tk.Button(frame1_1_L_ext, text="Repertoire\nde travail" ,command=choix_rep_L_ext, width=8, bg=COULEUR_INTERFACE)
bouton_rep_L_ext = tk.Button(frame1_1_L_ext, text=_("Fichier"),
                                  command=choix_fichier_L_ext, width=8, bg=COULEUR_INTERFACE)
bouton_execute_L_ext = tk.Button(
...
# coche_2D_L_ext=tk.Checkbutton(frame1_1_L_ext, text="Sortie 2D", variable=flag_2D_L_ext, command=change_flag_2D_L_ext, bg=COULEUR_INTERFACE)
coche_2D_L_ext = tk.Checkbutton(
...
# coche_3D_L_ext=tk.Checkbutton(frame1_1_L_ext, text="Sortie 3D", variable=flag_3D_L_ext, command=change_flag_3D_L_ext, bg=COULEUR_INTERFACE)
coche_3D_L_ext = tk.Checkbutton(
...
bouton_classification_L_ext = tk.Button(
...
text5_L_ext = tk.Label(frame2_L_ext, text=_("Position x (nm) : "), bg=COULEUR_INTERFACE)
text6_L_ext = tk.Label(
    frame2_L_ext, text=_("Position y (n° de spectre) : "), bg=COULEUR_INTERFACE)
text7_L_ext = tk.Label(frame2_L_ext, text=_("Position x (nm) : "), bg=COULEUR_INTERFACE)
text8_L_ext = tk.Label(
    frame2_L_ext, text=_("Position y (n° de spectre) : "), bg=COULEUR_INTERFACE)
...
frame2_1_L_ext = tk.Frame(frame2_L_ext, bg=COULEUR_INTERFACE)
...
coche_image_brute_L_ext = tk.Checkbutton(frame2_1_L_ext, text=_("Image brute\nspectres non\nnormalisés"), variable=flag_image_brute_L_ext,
...
coche_spectre_inclus_moyenne_L_ext = tk.Checkbutton(frame2_1_L_ext, text=_("Spectre inclus\ndans spectre\nmoyen"), variable=flag_spectre_inclus_moyenne_L_ext,
...
frame3_1_L_ext = tk.Frame(frame3_L_ext, bg=COULEUR_INTERFACE)
frame3_1_L_ext.grid(row=1, column=5, rowspan=4, sticky=tk.N)

text9_L_ext = tk.Label(frame3_1_L_ext, text=_("Du spectre n° :"), bg=COULEUR_INTERFACE)
text10_L_ext = tk.Label(frame3_1_L_ext, text=_("Au spectre n° :"), bg=COULEUR_INTERFACE)
...
entree9_L_ext = ttk.Spinbox(frame3_1_L_ext, from_=1, to=hauteur_canevas_spectres,
                                textvariable=variable_9_L_ext, command=retro_action_entree10_L_ext, width=5)
entree10_L_ext = ttk.Spinbox(frame3_1_L_ext, from_=1, to=hauteur_canevas_spectres,
...
bouton_extraction_L_ext = tk.Button(
...
coche_spectres_normalises_moyenne_L_ext = tk.Checkbutton(frame3_1_L_ext, text=_("Moyenne des\nspectres \nnormalisés"), variable=flag_spectres_normalises_moyenne_L_ext,
...
# Interface graphique : création des différentes zones/étapes (frames 1-2-3)
###################################################################################################
frame1_L_comp = tk.Frame(onglet3, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
frame2_L_comp = tk.Frame(onglet3, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
frame3_L_comp = tk.Frame(onglet3, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
...
lambda_texte_L_comp = tk.Label(
    frame1_L_comp, text="Lambda = " + str(format(lambda_texte_spectre_L_comp, "4.1f") + " nm"), bg=COULEUR_INTERFACE)
lambda_texte_L_comp.grid(row=2, column=5)

text1_L_comp = tk.Label(frame1_L_comp, text=_(
    "Numérateur borne inf (nm)"), bg=COULEUR_INTERFACE)
text2_L_comp = tk.Label(frame1_L_comp, text=_(
    "Numérateur borne sup (nm)"), bg=COULEUR_INTERFACE)
text3_L_comp = tk.Label(
    frame1_L_comp, text=_("Dénominateur borne inf (nm)"), bg=COULEUR_INTERFACE)
text4_L_comp = tk.Label(
...
entree1_L_comp = ttk.Spinbox(frame1_L_comp, from_=198, to=1013, increment=0.5,
                                 textvariable=variable_1_L_comp, command=deplace_ligne0_1_L_comp, foreground="red", width=5)
entree2_L_comp = ttk.Spinbox(frame1_L_comp, from_=198, to=1013, increment=0.5,
                                 textvariable=variable_2_L_comp, command=deplace_ligne0_2_L_comp, foreground="red", width=5)
entree3_L_comp = ttk.Spinbox(frame1_L_comp, from_=198, to=1013,  increment=0.5,
                                 textvariable=variable_3_L_comp, command=deplace_ligne0_3_L_comp, foreground="blue", width=5)
entree4_L_comp = ttk.Spinbox(frame1_L_comp, from_=198, to=1013,  increment=0.5,
...
coche_denominateur_L_comp = tk.Checkbutton(frame1_L_comp, text=_("Dénominateur ?"), variable=flag_denominateur_L_comp,
                                                command=change_flag_denominateur_L_comp, bg=COULEUR_INTERFACE)
coche_denominateur_L_comp.grid(row=3, column=5)

bouton_reset_L_comp = tk.Button(frame1_L_comp, text=_("Reset"),
                                     command=reset_tableau_L_comp,
                                     width=8, bg=COULEUR_INTERFACE)
bouton_reset_L_comp.grid(row=2, column=6, rowspan=2)

frame1_1_L_comp = tk.Frame(frame1_L_comp, bg=COULEUR_INTERFACE)
frame1_1_L_comp.grid(row=1, column=7, rowspan=3, sticky=tk.N)

text_zoom_L_comp = tk.Label(frame1_1_L_comp, text="Zoom : ", width=8, bg=COULEUR_INTERFACE)
...
# bouton_rep_L_comp=tk.Button(frame1_1_L_comp, text="Repertoire\nde travail" ,
#                                  command=choix_rep_L_comp, width=8, bg=COULEUR_INTERFACE)
bouton_rep_L_comp = tk.Button(frame1_1_L_comp, text=_("Fichier"),
                                   command=choix_fichier_L_comp, width=8, bg=COULEUR_INTERFACE)
bouton_execute_L_comp = tk.Button(frame1_1_L_comp, text=_("Exécute"),
...
coche_2D_L_comp = tk.Checkbutton(frame1_1_L_comp, text=_("Sortie 2D"),
                                      variable=flag_2D_L_comp, bg=COULEUR_INTERFACE)
# coche_2D_L_comp=tk.Checkbutton(
#     frame1_1_L_comp, text="Sortie 2D", variable=flag_2D_L_comp, command=change_flag_2D_L_comp, bg=COULEUR_INTERFACE)
coche_2D_L_comp.grid(row=6, column=1)

flag_3D_L_comp = tk.IntVar(value=flag_3D_init_L_comp)
coche_3D_L_comp = tk.Checkbutton(frame1_1_L_comp, text=_("Sortie 3D"),
                                      variable=flag_3D_L_comp, bg=COULEUR_INTERFACE)
# coche_3D_L_comp=tk.Checkbutton(
#     frame1_1_L_comp, text="Sortie 3D", variable=flag_3D_L_comp, command=change_flag_3D_L_comp, bg=COULEUR_INTERFACE)
coche_3D_L_comp.grid(row=7, column=1)

# image_classification=tk.PhotoImage(file=rep_LIBStick+"/LIBStick_datas/icons/Classification.png")
bouton_classification_L_comp = tk.Button(frame1_1_L_comp,
...
# text5_L_comp = tk.Label(frame2_L_comp, text="Position x (nm) : ", bg=COULEUR_INTERFACE)
# text6_L_comp = tk.Label(frame2_L_comp, text="Position y (n° de spectre) : ", bg=COULEUR_INTERFACE)
...
# entree5_L_comp=ttk.Spinbox(frame2_L_comp, from_=198, to=1013, textvariable=variable_5_L_comp, command=vars_5_6_to_coord1_L_comp, increment=0.5, width=8)
# entree6_L_comp=ttk.Spinbox(frame2_L_comp, from_=1, to=100, textvariable=variable_6_L_comp, command=vars_5_6_to_coord1_L_comp, width=8)
# entree5_L_comp.grid(row=2, column=2, sticky=tk.W)
# entree6_L_comp.grid(row=2, column=4, sticky=tk.W)

frame2_1_L_comp = tk.Frame(frame2_L_comp, bg=COULEUR_INTERFACE)
frame2_1_L_comp.grid(row=1, column=5, rowspan=3, sticky=tk.N)

# text7_L_comp=tk.Label(frame2_1_L_comp, text="Type de\nfichiers à\ncomparer :")
# text7_L_comp.grid(row=1, column=1)

flag_traitement_L_comp = tk.IntVar(value=flag_traitement_init_L_comp)
coche_traitement_L_comp = tk.Checkbutton(
...
# type_traitement_combobox_L_comp=ttk.Combobox(frame2_1_L_comp, textvariable=type_traitement_L_comp, width=10, values=["Echantillons différents", "Même échantillon"])
# type_traitement_combobox_L_comp.grid(row=2, column=1)

# text8_L_comp=tk.Label(frame2_1_L_comp, text="\nEchantillons différents :\nspectres moyens\nd'échantillons\ndifférents\n\nMême échantillon :\nspectres du même\néchantillon")
# text8_L_comp.grid(row=3, column=1)

flag_stat_L_comp = tk.IntVar(value=flag_stat_init_L_comp)
coche_stat_L_comp = tk.Checkbutton(
...
text5_L_comp = tk.Label(frame2_1_L_comp, text=_("\nPosition x (nm) : "), bg=COULEUR_INTERFACE)
text6_L_comp = tk.Label(
...
variable_5_L_comp = tk.DoubleVar(value=0)
variable_6_L_comp = tk.IntVar(value=0)
entree5_L_comp = ttk.Spinbox(frame2_1_L_comp, from_=198, to=1013,
                                 textvariable=variable_5_L_comp, command=vars_5_6_to_coord1_L_comp, increment=0.5, width=5)
entree6_L_comp = ttk.Spinbox(frame2_1_L_comp, from_=1, to=100,
...
texte_statistiques_L_comp = tk.Message(frame3_L_comp, bg=COULEUR_INTERFACE)
# texte_statistiques_L_comp.grid(row=1, column=3, sticky=tk.N)
...
# Interface graphique : création des différentes zones/étapes (frames 1-2-3)
###################################################################################################
frame1_L_ACP = tk.Frame(onglet4, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
frame2_L_ACP = tk.Frame(onglet4, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
frame3_L_ACP = tk.Frame(onglet4, borderwidth=2, relief=tk.RAISED, bg=COULEUR_INTERFACE)
...
lambda_texte_L_ACP = tk.Label(
    frame1_L_ACP, text="Lambda = " + str(format(lambda_texte_spectre_0_L_ACP, "4.1f") + " nm"), bg=COULEUR_INTERFACE)
lambda_texte_L_ACP.grid(row=2, column=5)

text1_L_ACP = tk.Label(frame1_L_ACP, text=_("Borne inf :"), bg=COULEUR_INTERFACE)
text2_L_ACP = tk.Label(frame1_L_ACP, text=_("Borne sup :"), bg=COULEUR_INTERFACE)
...
entree1_L_ACP = ttk.Spinbox(frame1_L_ACP, from_=198, to=1013, increment=0.5,
                                textvariable=variable_1_L_ACP, command=deplace_ligne0_1_L_ACP, width=5)
entree2_L_ACP = ttk.Spinbox(frame1_L_ACP, from_=198, to=1013, increment=0.5,
...
# bouton_reset_L_ACP=tk.Button(frame1_L_ACP, text="Reset", command=reset_tableau_L_ACP, width=8, bg=COULEUR_INTERFACE)
# bouton_reset_L_ACP.grid(row=2, column=6, rowspan=2)

frame1_1_L_ACP = tk.Frame(frame1_L_ACP, bg=COULEUR_INTERFACE)
frame1_1_L_ACP.grid(row=1, column=7, rowspan=3, sticky=tk.N)

text_zoom_L_ACP = tk.Label(frame1_1_L_ACP, text="Zoom : ", width=8, bg=COULEUR_INTERFACE)
...
entree_zoom_inf_L_ACP = ttk.Spinbox(frame1_1_L_ACP, from_=198, to=1013, increment=1,
                                        textvariable=variable_zoom_inf_L_ACP, command=change_zoom_inf_L_ACP, width=8)
entree_zoom_sup_L_ACP = ttk.Spinbox(frame1_1_L_ACP, from_=198, to=1013, increment=1,
...
frame2_1_L_ACP = tk.Frame(frame2_L_ACP, bg=COULEUR_INTERFACE)
frame2_1_L_ACP.grid(row=1, column=7, rowspan=3, sticky=tk.N)

flag_normalise_L_ACP = tk.BooleanVar(value=True)
coche_normalise_L_ACP = tk.Checkbutton(frame2_1_L_ACP, text=_("Spectres normalisés"), variable=flag_normalise_L_ACP,
                                            command=change_flag_traitement_L_ACP, bg=COULEUR_INTERFACE)
coche_normalise_L_ACP.grid(row=1, column=1, sticky=tk.W, columnspan=2)

flag_centre_reduit_L_ACP = tk.BooleanVar(value=False)
coche_centre_reduit_L_ACP = tk.Checkbutton(frame2_1_L_ACP, text=_("Centré reduit"), variable=flag_centre_reduit_L_ACP,
                                                command=change_flag_traitement_L_ACP, bg=COULEUR_INTERFACE)
coche_centre_reduit_L_ACP.grid(row=2, column=1, sticky=tk.W, columnspan=2)

bouton_execute_L_ACP = tk.Button(frame2_1_L_ACP, text=_("ACP"),
                                      command=execute_ACP_L_ACP, state="disable", width=6, bg=COULEUR_INTERFACE)
bouton_execute_L_ACP.grid(row=3, column=1, sticky=tk.W)

bouton_applique_L_ACP = tk.Button(frame2_1_L_ACP, text=_("+ ind. supp."),
                                       command=applique_ACP_L_ACP, state="disable", width=6, bg=COULEUR_INTERFACE)
bouton_applique_L_ACP.grid(row=3, column=2, sticky=tk.W)

bouton_ouvre_L_ACP = tk.Button(
...
text_dim_L_ACP = tk.Label(frame2_1_L_ACP, text=_("Dimensions : "), bg=COULEUR_INTERFACE)
...
coche_echelle_L_ACP = tk.Checkbutton(
...
coche_eboulis_L_ACP = tk.Checkbutton(
...
# coche_calcul_L_ACP=tk.Checkbutton(frame2_1_L_ACP, text="fanalysis ?", variable=flag_calcul_L_ACP, bg=COULEUR_INTERFACE)
...
frame3_1_L_ACP = tk.Frame(frame3_L_ACP, bg=COULEUR_INTERFACE)
frame3_1_L_ACP.grid(row=1, column=7, rowspan=3, sticky=tk.N)

bouton_enregistre_L_ACP = tk.Button(frame3_1_L_ACP, text=_("Enregistrer \nfacteurs \nde l'ACP"),
