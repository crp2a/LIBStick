...
class case_classification(tkinter.Button) :
    def __init__(self, boss, nom, symbole,  Z, ligne, colonne, couleur):
        texte="    "+str(Z)+"\n"+symbole
        tkinter.Button.__init__(self, boss, text=texte, bg=couleur, command=lambda : self.affiche_pics(nom, symbole,Z,couleur))
...
def lit_tableau_periodique() :
    DataFrame_tableau_periodique= pandas.read_table("./LIBStick_classification.tsv")
...
fenetre=tkinter.Tk()
frame1=tkinter.Frame(fenetre, bg="linen")
frame1.pack()
...
bouton_affichage=tkinter.Button(frame1, width=taille_case[0]*2, height=taille_case[1]*2)
